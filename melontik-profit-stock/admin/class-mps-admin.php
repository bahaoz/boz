...
class MPS_Admin {
	...
	public function register_menu() {
		...
		add_submenu_page(
			'mps-profit',
			__( 'Kanallar', 'melontik-profit-stock' ),
			__( 'Kanallar', 'melontik-profit-stock' ),
			'manage_woocommerce',
			'mps-channels',
	...
	public function register_settings() {
		...
		register_setting( 'mps_channel_settings_group', 'mps_channel_settings', array( $this, 'sanitize_channel_settings' ) );
	...
	public function sanitize_channel_settings( $input ) {
		$channels = array( 'woo', 'trendyol', 'hepsiburada' );
		$fields   = array(
			'commission_rate',
			'service_fee_rate',
			'commission_vat_rate',
			'shipping_deduction_rate',
			'payment_fee_rate',
			'coupon_support_rate',
			'other_fee_rate',
		);
		...
		foreach ( $channels as $channel ) {
			$sanitized[ $channel ] = array();

			foreach ( $fields as $field ) {
				$value = isset( $input[ $channel ][ $field ] ) ? $input[ $channel ][ $field ] : 0;
				$sanitized[ $channel ][ $field ] = (float) $value;
			}

			$sanitized[ $channel ]['commission_vat_included'] = ! empty( $input[ $channel ]['commission_vat_included'] ) ? 1 : 0;
		}
	...
	public function render_channels() {
		...
		require MPS_PLUGIN_DIR . 'admin/views/channels.php';
	}
