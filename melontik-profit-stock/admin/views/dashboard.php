...
$total_cost        = 0;
$total_profit      = 0;
$total_gross_incl_tax = 0;
$total_tax            = 0;
$total_net_excl_tax   = 0;
$total_fees           = 0;
$total_cogs           = 0;
...
foreach ( $orders as $order ) {
	...
	$total_cost        += $calc['total_fees'] + $calc['cogs'];
	$total_profit      += $calc['net_profit'];
	$total_gross_incl_tax += $calc['gross_incl_tax'];
	$total_tax            += $calc['tax_total'];
	$total_net_excl_tax   += $calc['net_excl_tax'];
	$total_fees           += $calc['total_fees'];
	$total_cogs           += $calc['cogs'];
	...
	$order_rows[] = array(
		...
		'shipping_deduction' => $calc['shipping_deduction'],
		'coupon_cost'    => $calc['coupon_cost'],
	...
	foreach ( $order->get_items() as $item ) {
		...
		$product_rows[ $key ]['total_cost']  += ( $calc['total_fees'] + $calc['cogs'] ) * $share;
...
?>
<div class="wrap">
	<h1><?php esc_html_e( 'Karlılık Raporları', 'melontik-profit-stock' ); ?>
		<span style="font-size:12px; font-weight:normal; margin-left:8px;">
			<?php echo esc_html( sprintf( __( 'Sürüm %s', 'melontik-profit-stock' ), MPS_VERSION ) ); ?>
		</span>
	</h1>
	...
	<div style="display:flex; gap:20px; flex-wrap:wrap;">
		<div class="card">
			<h3><?php esc_html_e( 'Brüt Satış (KDV Dahil)', 'melontik-profit-stock' ); ?></h3>
			<p><?php echo wp_kses_post( wc_price( $total_gross_incl_tax ) ); ?></p>
		</div>
		<div class="card">
			<h3><?php esc_html_e( 'KDV', 'melontik-profit-stock' ); ?></h3>
			<p><?php echo wp_kses_post( wc_price( $total_tax ) ); ?></p>
		</div>
		<div class="card">
			<h3><?php esc_html_e( 'Vergi Hariç Satış', 'melontik-profit-stock' ); ?></h3>
			<p><?php echo wp_kses_post( wc_price( $total_net_excl_tax ) ); ?></p>
		</div>
		<div class="card">
			<h3><?php esc_html_e( 'Toplam Kesinti', 'melontik-profit-stock' ); ?></h3>
			<p><?php echo wp_kses_post( wc_price( $total_fees ) ); ?></p>
		</div>
		<div class="card">
			<h3><?php esc_html_e( 'Net Gelir', 'melontik-profit-stock' ); ?></h3>
			<p><?php echo wp_kses_post( wc_price( $total_net_revenue ) ); ?></p>
		</div>
		<div class="card">
			<h3><?php esc_html_e( 'COGS', 'melontik-profit-stock' ); ?></h3>
			<p><?php echo wp_kses_post( wc_price( $total_cogs ) ); ?></p>
		</div>
		<div class="card">
	...
	<table class="widefat striped">
		<thead>
			<tr>
				...
				<th><?php esc_html_e( 'Komisyon', 'melontik-profit-stock' ); ?></th>
				<th><?php esc_html_e( 'Hizmet Bedeli', 'melontik-profit-stock' ); ?></th>
				<th><?php esc_html_e( 'Komisyon KDV', 'melontik-profit-stock' ); ?></th>
				<th><?php esc_html_e( 'Kargo Kesintisi', 'melontik-profit-stock' ); ?></th>
				<th><?php esc_html_e( 'Kupon/Kampanya', 'melontik-profit-stock' ); ?></th>
				<th><?php esc_html_e( 'Diğer', 'melontik-profit-stock' ); ?></th>
		...
		<tbody>
			<?php if ( empty( $order_rows_paged ) ) : ?>
				<tr><td colspan="14"><?php esc_html_e( 'Kayıt bulunamadı.', 'melontik-profit-stock' ); ?></td></tr>
			<?php else : ?>
				<?php foreach ( $order_rows_paged as $row ) : ?>
					<tr>
						...
						<td><?php echo wp_kses_post( wc_price( $row['commission_fee'] ) ); ?></td>
						<td><?php echo wp_kses_post( wc_price( $row['service_fee'] ) ); ?></td>
						<td><?php echo wp_kses_post( wc_price( $row['commission_vat'] ) ); ?></td>
						<td><?php echo wp_kses_post( wc_price( $row['shipping_deduction'] ) ); ?></td>
						<td><?php echo wp_kses_post( wc_price( $row['coupon_cost'] ) ); ?></td>
						<td><?php echo wp_kses_post( wc_price( $row['other_fee'] ) ); ?></td>
