...
class MPS_Calculator {
	public function calculate_order( WC_Order $order, array $settings ) {
		...
		$net_revenue = ( $gross_sales + $shipping_collected ) - $discounts - $tax_total - $payment_fee;
		...
		foreach ( $order->get_items() as $item ) {
			...
			$unit_cost = $purchase_cost + $packaging_cost + $fixed_overhead;
		...
		$commission_fee = (float) $order->get_meta( '_mps_commission_fee', true );

		$total_cost = $product_costs + $shipping_cost + $commission_fee + $payment_fee;
